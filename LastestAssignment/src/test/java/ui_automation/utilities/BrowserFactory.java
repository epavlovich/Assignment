            ...
            try {
                if (driver == null) {
                    if(System.getProperty("browser")==null){
                        // it is similar to ==> Webdriver driver = new ChromeDriver();
                        WebDriverManager.chromedriver().setup();
                        // Create a hashmap to store key and value of config
                        ...
                        // Create a chromeOption
                        ChromeOptions options = new ChromeOptions();
                        options.addArguments("--ignore-ssl-errors=yes");
                        options.addArguments("--ignore-certificate-errors");
                        options.setExperimentalOption("prefs", chromePrefs);
                        // Will set above options to chomedriver
                        ...
                        switch (System.getProperty("browser")) {
                            ...
                            case "firefoxRemote":
                                ...
                                try {
                                    driver = new RemoteWebDriver(new URL("http://18.232.138.138:4444/wd/hub"), firOptions);
                                } catch (MalformedURLException e) {
                                    e.printStackTrace();
                                }
                                break;
